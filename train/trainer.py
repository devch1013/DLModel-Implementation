...
class Trainer:
    ...
    def enable_ckpt(self, save_dir: str, ckpt_save_num: int):
    ...
    def enable_tensorboard(self, log_dir: str = None, save_image_log=False, save_image_num=1):
        assert self.cfg["validate"]["batch-size"] >= save_image_num
        self.save_image_log = save_image_log
        self.save_image_num = save_image_num
    ...
    def set_model(self, model_class, state_dict=None):
        """
        get model class and set model with config information
        """
    ...
    def _validate(self, current_epoch):
        ...
        if self.writer is not None:
            ...
            if self.save_image_log:
                origin_img = torchvision.utils.make_grid(data[: self.save_image_num], pad_value=0.5)
                if self.multi_output:
                    output_tensor = []
                    outputs = list(outputs)
                    outputs.append(output)
                    for o in outputs:
                        output_tensor.append(o[: self.save_image_num].to(dtype=torch.int32))
                    output_tensor = torch.concat(output_tensor, dim=0)
                else:
                    output_tensor = outputs[: self.save_image_num]
                data_img = torchvision.utils.make_grid(output_tensor, nrow=5, pad_value=0.5)
                mask_img = torchvision.utils.make_grid(target[: self.save_image_num], pad_value=0.5)
        ...
        if self.save_ckpt:
            ...
            if len(self.best_ckpt) < self.ckpt_save_num:
                torch.save(
